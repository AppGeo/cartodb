...
var debug = require('debug')('cartodb:compiler');
var Raw = require('./raw');
...
assign(QueryCompiler.prototype, {
  ...
  select: function select() {
    ...
    return statements.filter(function (item) {
      return item;
    }).join(' ');
  },
  ...
  insert: function insert() {
    ...
    this.__insertValues = insertData.values;
    if (typeof insertData === 'string') {
      ...
      if (insertData.columns.length) {
        ...
        sql += ') values';
        const insert = '(' + this.formatter.parameterize(insertData.columns) + ')';
        sql += repeat(insert, insertData.rows).join(',');
    ...
    var out = {
      sql: sql,
      bindings: insertData.values
    };
    if (out.sql !== '') {
      out.returning = this.single.returning;
    }
    return out;
  },
    ...
    while (++i < joins.length) {
      ...
      if (join.joinType === 'raw') {
        ...
        while (++ii < join.clauses.length) {
          ...
          if (clause[3]) {
            sql += ' ' + this.formatter.operator(clause[3]);
          }
          if (clause[4]) {
            sql += ' ' + this.formatter.wrap(clause[4]);
          }
        }
      }
    }
  ...
  lock: function lock() {
    if (this.single.lock) {
      if (!this.client.transacting) {
        debug('You are attempting to perform a "lock" command outside of a transaction.');
      } else {
  ...
  whereBetween: function whereBetween(statement) {
    return this.formatter.wrap(statement.column) + ' ' + this._not(statement, 'between') + ' ' + statement.value.map(this.formatter.parameter, this.formatter).join(' and ');
  },
